...
parser.add_argument('--plot', '-p', type=str, default="13000_no_hadr_12112019_MMHT2014lo68cl.yoda")
parser.add_argument('--comp', '-c', type=str, default="900_no_hadr_12112019_MMHT2014lo68cl.yoda")
...
def Myplot_hist_on_axes_1d(axmain, axratio, h, href, default_color="red", default_linestyle="-",href_default_color="blue", href_default_linestyle=".", mylabel = "", href_mylabel = "",logopt=0,myxlabel="",myylabel="",extralabel="",extralabel2=""):
    h = mk_numpyhist(h)
    ...
    artists = axmain.step(np.append(h.xmin, h.xmax[-1]), np.append(h.y, h.y[-1]), label = mylabel,where="post", color=default_color, linestyle=default_linestyle, linewidth=lwidth)
    artists = axmain.step(np.append(href.xmin, href.xmax[-1]), np.append(href.y, href.y[-1]), label = href_mylabel,where="post", color=href_default_color, linestyle=href_default_linestyle, linewidth=lwidth)
    #extralabel="hi"
    #extralabel2="hi2"
...
for i in range(len(ListOfFractionPt)):
    ...
    fig, axmain, axratio = MyPlot(None, ListOfFractionPt[i])
    print "TYPE: ", type(h)
    firstplot = Myplot_hist_on_axes_1d(axmain, axratio, h, href, COLORS[0], LSTYLES[0], COLORS[1], LSTYLES[1], "13000 GeV","900 GeV",logopt=0,myylabel="Events",myxlabel="Pt [GeV]") 
...
for i in range(len(ListOfPDGID)):
    ...
    firstplot = Myplot_hist_on_axes_1d(axmain, axratio, h, href, COLORS[0], LSTYLES[0], COLORS[1], LSTYLES[1], "13000 GeV","900 GeV",logopt=1,myylabel="Events",myxlabel="pdgid of partons") 
    #def Myplot_hist_on_axes_1d(axmain, axratio, h, href, default_color="red", default_linestyle="-",href_default_color="blue", href_default_linestyle=".", mylabel = "",logopt=0,myxlabel="",myylabel="",extralabel="",extralabel2=""):
    #plot = "second"
    #aa = Myplot_hist_on_axes_1d(axmain, axratio, h, href, COLORS[1], LSTYLES[1], plot,1,0,myxlabel="$\\lambda_0^0$",number=1, extralabel="hi")
    #plt.show()
...
for i in range(len(ListOfMulti)):
    ...
    fig, axmain, axratio = MyPlot(None, ListOfMulti[i])
    firstplot = Myplot_hist_on_axes_1d(axmain, axratio, h, href, COLORS[0], LSTYLES[0], COLORS[1], LSTYLES[1], "13000 GeV","900 GeV",logopt=1,myylabel="Events",myxlabel="Number of partons") 
    #def Myplot_hist_on_axes_1d(axmain, axratio, h, href, default_color="red", default_linestyle="-",href_default_color="blue", href_default_linestyle=".", mylabel = "",logopt=0,myxlabel="",myylabel="",extralabel="",extralabel2=""):
    #plot = "second"
    #aa = Myplot_hist_on_axes_1d(axmain, axratio, h, href, COLORS[1], LSTYLES[1], plot,1,0,myxlabel="$\\lambda_0^0$",number=1, extralabel="hi") 
    #plt.show()
